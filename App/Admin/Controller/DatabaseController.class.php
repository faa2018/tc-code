...
class DatabaseController extends AdminBaseController{
     ...
     * 还原数据库
     */
    public function import($time = null, $part = null, $start = null){
            if(is_numeric($time) && is_null($part) && is_null($start)){ //初始化
                ...
                if(count($list) === $last[0]){
                    ...
                    $this->success('初始化完成！请耐心等待...', '', array('part' => 1, 'start' => 0));
                } else {
                ...
                if(false === $start){
                    ...
                    if($start[1]){
                        $rate = floor(100 * ($start[0] / $start[1]));
                        $this->success("正在还原2...#{$part} ({$rate}%)", '', $data);
                    } else {
                        $data['gz'] = 1;
                        $this->success("正在还原3...#{$part}", '', $data);
                    }
                }
