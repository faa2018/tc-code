...
//******************用户-用户组*******************
    ...
    public function check_user(){
        ...
        if(empty($username)){
            ...
            $user_data=M('AdminUser')->where(array('username'=>$username))->select();
        }
        $assign=array(
    ...
    public function add_admin(){
        if(IS_POST){
            $data=I('post.');
            $result=D('AdminUser')->addData($data);
            if($result){
    ...
    public function deleteAdmin(){
    	$id = I('id');
    	$list = M('Admin_user')->where(array('id'=>$id))->delete();
    	if($list){
    		$this->success('删除成功',U("Rule/admin_user_list"));
    	}else{
    		$this->error('删除失败');
    	}
    }
